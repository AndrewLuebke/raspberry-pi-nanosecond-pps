#!/bin/bash
# One line per minute: UTC | SoC temp m°C | CPU2 kHz | SHT35 box temp °C + RH % (or - -) | chrony PPS offset,stddev (sourcestats) | pps_warm fires + loop stats
...
printf '%s %s %s %s %s %s\n' "$(date -u +%FT%T)" "$(cat /sys/class/thermal/thermal_zone0/temp)" "$(cat /sys/devices/system/cpu/cpu2/cpufreq/scaling_cur_freq)" "$sht" \
  "$(chronyc sourcestats 2>/dev/null | awk '$1=="PPS"{print $7, $8}' | tr ' ' ',')" \
